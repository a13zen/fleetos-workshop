"""
Characterisation tests for the legacy FleetTracker maintenance functions.
...
# The legacy app imports db_utils as a bare name (no package prefix), so we
# must also make sure the legacy directory is the working directory or on the
# path so that import resolves correctly.
_orig_cwd = os.getcwd()
...
# ---------------------------------------------------------------------------
# Helper: build a vehicle dict as db_utils returns it (all values are strings,
# matching the raw CSV output).
# ---------------------------------------------------------------------------
...
class TestAllVehiclesFullSuite:
    ...
    def test_full_golden_dataset(self, vid, cls, mileage, exp_date, exp_km, exp_status, exp_prio):
        ...
        due_date, due_km = legacy_app.calc_next_service(v, TODAY)
        assert due_date == exp_date, f"{vid}: due_date mismatch"
        assert due_km == exp_km, f"{vid}: due_km mismatch"
        assert legacy_app.calc_status(v, TODAY) == exp_status, f"{vid}: status mismatch"
        assert legacy_app.calc_priority(v, TODAY) == exp_prio, f"{vid}: priority mismatch"
